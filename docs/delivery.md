...
工作区已将一级分类调整为「学业／生活／我喜欢的」，对应 `study`／`life`／`favorites`，文章数量为 3／1／0；旧 `ai`、`coding`、`research` 查询参数兼容映射到 `study`。本轮完成本地回归与截图归档，未推送、未部署；下方线上交付与演示仍记录当前已发布版本，下一次发布时再同步新分类。
